...
export default async function scraper (options = {}) {
  ...
  const countySearch = county.toUpperCase().replace('-', ' ')
  ...
  documentTypeInput.fill('1') // identifier for standard deed
  ...
  const retrievedRecordsElement = await page.$('#results > div:nth-child(4) > div > div.h5.ng-binding')
  ...
  await repeat(totalRecords, async () => {
    ...
    const data = {
      ...
      date: row.Date,
      grantor: document.grantors,
      grantee: document.grantees,
      block: row.Block,
      lot,
      qualifier,
    ...
    try {
      if (data.block && data.lot) {
      ...
      } else if (data.legal && data.legal.length) {
        response = await parcels.request({
          address: data.legal,
          county: countySearch,
          municipality: data.municipality
        ...
        errors.push({
          error_message: 'no address or block and lot',
          data,
          document,
          row,
          response,
